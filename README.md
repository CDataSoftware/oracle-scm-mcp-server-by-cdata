...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/oraclescm/download/jdbc](https://www.cdata.com/drivers/oraclescm/download/jdbc/)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Oracle SCM\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Oracle SCM/`
    * Run the command `java -jar cdata.jdbc.oraclescm.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.oraclescm.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `oracle-scm.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=oraclescm
      ServerName=CDataOracleSCM
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.oraclescm.jar
      DriverClass=cdata.jdbc.oraclescm.OracleSCMDriver
      JdbcUrl=jdbc:oraclescm:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
